...
TTS_VOICE_OPTIONS: Dict[str, str] = {
    "中文女聲（台灣，活潑）": "zh-TW-HsiaoYuNeural",
    "中文女聲（台灣，親切）": "zh-TW-HsiaoChenNeural",
    "中文男聲（台灣）": "zh-TW-YunJheNeural",
    "中文女聲（大陸，活潑）": "zh-CN-XiaoyiNeural",
    "中文女聲（大陸，溫暖）": "zh-CN-XiaoxiaoNeural",
    "中文男聲（大陸，陽光）": "zh-CN-YunxiNeural",
}
TTS_RATE_OPTIONS = ["-10%", "0%", "+5%", "+10%", "+20%"]
...
def _configure_runtime_environment() -> None:
    ...
    for candidate in ffmpeg_candidates:
        ffmpeg_path = os.path.join(base_path, candidate)
        if os.path.isfile(ffmpeg_path):
            current_path = os.environ.get("PATH", "")
            if base_path not in current_path.split(os.pathsep):
                os.environ["PATH"] = os.pathsep.join([base_path, current_path]) if current_path else base_path
            break
...
class WhisperApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.tts_voice_label_var = tk.StringVar(value="中文女聲（台灣，活潑）")
        ...
        self.tts_preview_mode_var = tk.StringVar(value="優化預覽")
    ...
    def _build_ui(self) -> None:
        ...
        tts_frame = ttk.Frame(notebook, padding=20)
    ...
    def _build_tts_tab(self, frame: ttk.Frame) -> None:
        ...
        ttk.Label(
            frame,
            text="貼上旁白文字後即可輸出 mp3。可先做朗讀優化預覽，長文會自動分段合成再合併。",
            foreground="#444",
            wraplength=700,
    ...
    def paste_tts_text(self) -> None:
        ...
        if not clipboard:
            messagebox.showwarning("提醒", "剪貼簿目前沒有可貼上的文字")
            return
        self.tts_text.insert(tk.INSERT, clipboard)
        self.tts_text.focus_set()
        self.refresh_tts_preview()
    ...
    def _optimize_tts_text(self, text: str) -> str:
        ...
        optimized = re.sub(
            ...
            r"(?:\s*[~-]\s*(?P<end_hour>[01]?\d|2[0-3]):(?P<end_minute>[0-5]\d))?",
            self._spoken_time,
            optimized,
        )
        optimized = re.sub(
    ...
    def refresh_tts_preview(self) -> None:
        ...
        if self.tts_preview_mode_var.get() == "原文預覽":
            ...
            preview_text = self._optimize_tts_text(source)
    ...
    def _default_tts_filename(self) -> str:
        ...
        return f"{self._sanitize_filename(sample)}.mp3"
