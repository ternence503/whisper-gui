...
- 選擇中文男聲或女聲
- 設定語速與音高
- 選擇輸出模式：原文直出 / 2 分鐘精簡版 / 短影音版
- 使用朗讀優化預覽原文 / 目前實際輸出文字
- 輸出為 `mp3`

長文會自動分段合成後再合併，避免一次送出過長內容導致失敗。

目前語速選項為 `-10%`、`0%`、`+5%`、`+10%`、`+20%`，預設值為 `+5%`。

如果原稿時長需要大幅壓縮，例如約 5 分鐘縮到約 2 分鐘，不建議只靠拉高語速；現在可先用內建精簡模式自動濃縮旁白，再搭配適度加速。這個精簡模式屬於本地規則式整理，會優先保留日期、時間、地點與行動呼籲，適合先快速產出可用版本，再由人手微調。
